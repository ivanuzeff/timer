...
const playAlarm = (ctx: AudioContext, time: number) => {
...
const playDigital = (ctx: AudioContext, time: number) => {
...
const playCosmic = (ctx: AudioContext, time: number) => {
  // Long sweep
  const osc = ctx.createOscillator();
  const gain = ctx.createGain();
  osc.connect(gain);
...
export const playTimerSound = (type: SoundType) => {
  ...
  switch (type) {
    case 'alarm':
      ...
      playAlarm(ctx, now);
      playAlarm(ctx, now + 1.6);
      playAlarm(ctx, now + 3.2);
      break;
    case 'digital':
      // Repeat quickly
      playDigital(ctx, now);
      playDigital(ctx, now + 0.5);
      playDigital(ctx, now + 1.0);
      playDigital(ctx, now + 1.5);
      break;
    case 'cosmic':
      playCosmic(ctx, now);
      break;
    ...
    default:
      ...
      // Double beep
      setTimeout(() => {
          if (audioContext) playBeep(audioContext, audioContext.currentTime);
      }, 200);
      break;
  }
};
